...
namespace Dot {
	LightController::LightController(const glm::vec3& position, const glm::vec3& color)
	{
		...
		m_Mesh = AssetManager::Get()->GetStaticMesh("Grass");
		m_Model = CalculateModel(m_Light->GetPosition());

		//Wrapper::Create("Light", glm::vec2(500, 500), glm::vec2(300, 300),2);
		//Gui::Get()->EnableWrapper("Light");
		//{
		//	Slider::Create("Strength", glm::vec2(50, 50), glm::vec2(200, 20), &m_Light->GetStrength(),1);
