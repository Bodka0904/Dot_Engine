...
namespace Dot {
	...
	void IndexedModel::CalcNormals()
	{
		for (unsigned int i = 0; i < indices.size(); i += 3)
		{
			...
			glm::vec3 v1 = m_vertex[i1].vertice - m_vertex[i0].vertice;
			glm::vec3 v2 = m_vertex[i2].vertice - m_vertex[i0].vertice;
	...
	IndexedModel OBJModel::ToIndexedModel()
	{
		...
		for (unsigned int i = 0; i < numIndices; i++)
		{
			...
			if (it == normalModelIndexMap.end())
			{
				...
				normalModelIndex = normalModel.m_vertex.size();

				normalModelIndexMap.insert(std::pair<OBJIndex, unsigned int>(*currentIndex, normalModelIndex));

				normalModel.m_vertex.push_back(VertexTexture(currentPosition, currentNormal, currentTexCoord));
			...
			{
				resultModelIndex = result.m_vertex.size();
		...
		if (!hasNormals)
		{
			...
			for (unsigned int i = 0; i < result.m_vertex.size(); i++)
			{
			...
			if (testIndex->vertexIndex == currentIndex->vertexIndex)
			{
				...
				for (unsigned int i = countStart; i < indexLookup.size() - countStart; i++)
				{
						...
						&& (!hasNormals || possibleIndex->normalIndex == currentIndex->normalIndex))
					{
						...
						for (unsigned int j = 0; j < result.m_vertex.size(); j++)
						{
							if (currentPosition == result.m_vertex[j].vertice
								&& ((!hasUVs || currentTexCoord == result.m_vertex[j].texcoord)
									&& (!hasNormals || currentNormal == result.m_vertex[j].normal)))
							{
								return j;
							}
						}
